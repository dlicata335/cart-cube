  ...
  private
    module GlueComInternal {l1 l : Level} {Γ : Set l1}
      ...
      use-incoh : ∀ (p : I → Γ) (s s' : I) (β : Set) {{cβ : Cofib β}} (t : (z : I) → β → (GlueF α T B f o p) z) (b : ((GlueF α T B f o p) s) [ β ↦ (t s) ]) 
                  → _[_↦_,_↦_] ((GlueF α T B f o p) s') (β ∨ ∀i (fst o α o p)) {{ Cofib∨ cβ (Cofib∀ (\ x → snd (α (p x)))) }} 
                                 (case (λ pβ → t s' pβ)
                                   (λ p∀α →
                                   ...
                                   (s == s') (⇒ (fst b))
      use-incoh p s s' β {{cβ}} t b = icom p s s' (β ∨ ∀i (fst o α o p)) {{ Cofib∨ cβ (Cofib∀ (\ x → snd (α (p x)))) }} 
                                (\ w → case (\ pβ → t w pβ) 
                                             (\ p∀α → coe (! (Glue-α _ {{ (snd (α (p w))) }} _ _ _ (p∀α w)))
                                                          (fst (fill-Glue-fiber p p∀α s w β {{ cβ }} t b)))
                                            (\ pβ p∀α →  move-transport-right (\ X → X) (Glue-α (fst (α (p w))) {{ (snd (α (p w))) }}  (T (p w)) (B (p w)) (f (p w)) (p∀α w)) (fst (snd (fill-Glue-fiber p p∀α s w β {{ cβ }} t b)) pβ) ))
                                (fst b , 
                                 ∨-elim _ (snd b) (\ p∀α → move-transport-left! (\ X → X) (Glue-α (fst (α (p s))) {{ (snd (α (p s))) }} (T (p s)) (B (p s)) (f (p s)) (p∀α s)) (! (snd (snd (fill-Glue-fiber p p∀α s s β {{ cβ }} t b)) id))) (\ _ _ → uip)) where
  ...
  module GlueCom  where 
      ...
      abstract
        ...
        comGlue-∀α : {l1 l : Level} {Γ : Set l1}
                  ...
                  (comT : relCom (\ (p : Σ \ (x : Γ) → fst (α x)) → T (fst p) (snd p)))
                  (comB : relCom B)
                  (wCoeGlue : relWCoe (GlueF α T B f)) (p : _) (p∀α : ∀i (fst o α o p)) → ∀ s s' β {{cβ}} t b → 
        ...
        GlueF-stable : ∀ {l} {Γ : Set l} {Δ : Set l} {θ : Δ → Γ}
          (α : Γ → Cofibs)
          (T : (x : Γ) → fst (α x) → Set l)
          (B : Γ → Set l)
          (f : (x : Γ) (u : fst (α x)) → T x u → B x)
